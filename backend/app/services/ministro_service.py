...
def atualizar(db: Session, ministro_id: int, data: MinistroIn) -> MinistroOut | None:
    ...
    if not m:
        return None
    normalized_email = auth_service.validate_email(data.email)
    duplicate = db.query(Ministro).filter(Ministro.email == normalized_email, Ministro.id != m.id).first()
    if duplicate:
        raise ValueError("Já existe um ministro com este e-mail.")
    if m.vinculo_usuario:
        ...
        conflicting_user = (
            db.query(Usuario)
            .filter(Usuario.email == normalized_email, Usuario.id != linked_user.id)
            .first()
        )
        if conflicting_user:
