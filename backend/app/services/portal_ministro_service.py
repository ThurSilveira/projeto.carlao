...
def _event_finished(event: Evento) -> bool:
    try:
        timezone = ZoneInfo(os.getenv("GOOGLE_CALENDAR_TIMEZONE", "America/Sao_Paulo"))
    ...
    try:
        ...
        duration_minutes = int(os.getenv("GOOGLE_CALENDAR_EVENT_DURATION_MINUTES", "120"))
    except (TypeError, ValueError) as exc:
        raise ValueError("O evento possui horário ou duração inválidos.") from exc
    event_end = datetime.combine(event.data, event_time, timezone) + timedelta(minutes=duration_minutes)
...
def calendar(
    ...
    feedback_event_ids = {
        row[0]
        for row in (
            db.query(Feedback.evento_id)
            .filter(Feedback.ministro_id == minister.id, Feedback.evento_id.in_(event_ids))
            .all()
        )
    }
...
def create_unavailability(
    ...
    auditoria_service.registrar(
        ...
        "CRIADO",
        None,
        f"Ministro {minister.id} — {data.data}",
        str(user.id),
    )
    db.commit()
    db.refresh(entity)
...
def list_feedback(db: Session, minister: Ministro) -> list[FeedbackMinistroOut]:
    rows = (
        db.query(Feedback)
        .filter(Feedback.ministro_id == minister.id)
        .order_by(Feedback.data_envio.desc())
        .all()
    )
...
def create_feedback(
    ...
    auditoria_service.registrar(
        ...
        "CRIADO",
        None,
        f"Evento {event.id} — ministro {minister.id}",
        str(user.id),
    )
    db.commit()
    db.refresh(feedback)
