...
def _get_user(db: Session, user_id: int) -> Usuario:
    ...
    if not user.acesso:
        raise ValueError("Usuário sem perfil de acesso. Reinicie o serviço para reparar a configuração.")
    return user
...
def _resolve_minister_link(
    ...
    if minister_id is None:
        raise ValueError("Informe ministroId para criar ou atualizar um usuário MINISTRO.")

    minister = db.get(Ministro, minister_id)
    if not minister:
        raise ValueError("Ministro não encontrado.")
    if not minister.ativo:
        raise ValueError("O ministro precisa estar ativo para receber acesso ao portal.")
    if auth_service.normalize_email(minister.email) != email:
        raise ValueError("O e-mail do usuário deve ser igual ao e-mail cadastrado para o ministro.")

    link_query = db.query(VinculoUsuarioMinistro).filter(VinculoUsuarioMinistro.ministro_id == minister.id)
    if ignore_user_id is not None:
        link_query = link_query.filter(VinculoUsuarioMinistro.usuario_id != ignore_user_id)
    if link_query.first():
...
def criar(db: Session, data: UsuarioAdminIn, actor: Usuario) -> UsuarioAdminOut:
    ...
    user = Usuario(
        nome=name,
        email=email,
        senha_hash=auth_service.hash_password(data.senha),
        ativo=data.ativo,
    )
    db.add(user)
    db.flush()
...
def redefinir_senha(db: Session, user_id: int, new_password: str, actor: Usuario) -> None:
    ...
    auditoria_service.registrar(
        ...
        "SENHA_REDEFINIDA",
        None,
        f"Sessões encerradas — {user.email}",
        str(actor.id),
    )
    db.commit()
