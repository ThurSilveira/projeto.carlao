...
class AuthServiceTest(unittest.TestCase):
    ...
    def test_bootstrap_login_session_and_logout(self):
        ...
        with patch.dict(os.environ, environment, clear=False):
            ...
            self.assertIsNone(
                auth_service.authenticate(self.db, user.email, "senha-incorreta", "127.0.0.1")
            )

            authenticated = auth_service.authenticate(
                self.db,
                user.email,
                environment["AUTH_ADMIN_PASSWORD"],
                "127.0.0.1",
            )
    ...
    def test_password_policy_and_rotation(self):
        user = Usuario(
            nome="Administrador",
        ...
        self.db.add(user)
        self.db.commit()
        old_session, old_token = auth_service.create_session(self.db, user)
        self.db.expunge(old_session)
        ...
        new_session, new_token = auth_service.change_password(
            self.db,
            user,
            "senha-atual-bastante-longa",
            "nova-senha-bastante-longa",
        )
        ...
        self.assertTrue(auth_service.verify_password("nova-senha-bastante-longa", user.senha_hash))

    def test_rejects_weak_bootstrap_password(self):
        environment = {
            "AUTH_ADMIN_NAME": "Administrador",
            "AUTH_ADMIN_EMAIL": "admin@example.com",
            "AUTH_ADMIN_PASSWORD": "curta",
        }
        with patch.dict(os.environ, environment, clear=False):
            with self.assertRaisesRegex(ValueError, "pelo menos 12 caracteres"):
                auth_service.bootstrap_admin(self.db)
